...
include_once(dirname(__FILE__) . "/settings.php");
	...
	if (Conf::$orig[0] != '/')
			Conf::$orig = dirname(__FILE__) . '/' . Conf::$orig . '/';
	...
	function save_image_file($file_type, $src, $dst, $preset, $w, $h)
	{
		...
		if ($w > Conf::$max_width || $h > Conf::$max_height)
				return error('too big');
		if (isset(Conf::$sizes) && (!isset(Conf::$sizes[$preset]) || !Conf::$sizes[$preset]))
		...
		$bgcolor = imagecolorallocate($img_dst, 0, 0, 0);
		imagefilledrectangle($img_dst, 0, 0, $w - 1, $h - 1, $bgcolor);
		$w0     = $w;
		$h0     = $h; // keep original width and height before scalling
		$w0_src = $w_src;
		$h0_src = $h_src; // keep original source width and height before cropping
		switch (Conf::$aspect_handling) {
				...
				case 2:
						...
						if ($w_src / $h_src > $ratio)
								$w_src = floor($h_src * $ratio);
						else
								$h_src = floor($w_src / $ratio);
		...
		//  save new image
		if (!call_user_func("image$file_type", $img_dst, $dst, Conf::$qaulity[$file_type]))
	...
	$fn = urldecode($fn);
	if (strstr($fn, '..'))
			error('double dots not allowed');

	$locks_d = dirname(__FILE__) . '/locks/';
	@mkdir(dirname($locks_d), 0777, true);
	if (!is_dir(dirname($locks_d)))
			die("could not create directory [$lock_d]\n");
	$lock_fn = $locks_d . md5($fn);
	$abs_dst = realpath(dirname(__FILE__)) . '/' . $fn;
	$a       = array();
	if (!preg_match('/^(([0-9]+)x([0-9]+)(?:_\w+)?)\/(.*(?:\.(png|jpe?g|gif)))\.(png|jpg)/i', $fn, $a))
			error('wrong file name syntax');
	$preset = $a[1];
	$w      = $a[2];
	$h      = $a[3];
	$e1     = strtolower($a[5]);
	$e2     = $a[6];
	if ($e2 == 'png' && $e1 != 'png' && $e1 != 'gif')
			error('wrong file name syntax');
	$src = Conf::$orig . '/' . $a[4];
	if (!file_exists($src))
			error('original file does not exists!');
