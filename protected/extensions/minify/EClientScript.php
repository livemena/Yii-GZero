 ...
 *
 * @link https://bitbucket.org/Koduc/eclientscript
 * @version 1.31
 *
 */
/**
...
class EClientScript extends CClientScript
{
        ...
        // Добавляем в игнор крупные файлы. Иначе получается ситуация, когда из-за мелкого js файлика генерируется новый и
        // огромный js
        public $scriptsForIgnore = array('jquery.js', 'jquery.min.js', 'jquery.ui.js', 'jquery-ui.min.js',
                'bootstrap.min.js');
        ...
        protected function combineScriptFiles($type = self::POS_HEAD)
        {
                ...
                foreach ($this->scriptFiles[$type] as $url)
                {
                        ...
                        if ($file === false)
                                ...
                                if(in_array(basename($file), $this->scriptsForIgnore)){
                                        $scriptFiles[$url] = $url;
                                } else {
                                ...
                                foreach ($toBeCombined as $url => $file)
                                {
                                        if(file_exists($file)){
                                                ...
                                                if ($contents)
                                                {
                                                        ...
                                                        $fileBuffer .= " ***/\n";
                                                        $fileBuffer .= $contents . ";\n\n";
                                                        // Добавляем в конец склейки ";", иначе может ломаться некоторый неправильный код,
                                                        // например, при использовании jQuery i18n
                                                }
                                        } else {
                                                $scriptFiles[$url] = $url;
                                        }
